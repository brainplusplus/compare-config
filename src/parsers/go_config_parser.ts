...
export const parseGoConfig = (envString: string): { [key: string]: string } => {
    ...
    const regex = /\b(?:[a-zA-Z_]+\.(Get(?:String|Bool|Int|Duration|Int64|Int32|Float64|IntSlice|SizeInBytes|StringMap|StringMapString|StringSlice|StringMapStringSlice|Time|Uint|Uint16|Uint32|Uint64|env))|getEnv\w*)\(\s*"([^"]+)/g;
